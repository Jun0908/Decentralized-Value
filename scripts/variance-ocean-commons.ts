...
for (const e of EFFORT)
  for (const r of RESERVE)
    for (const c of CONTRACTS) GRID.push({ effortFraction: e, reserve: r, contracts: c });

function background(s: OceanScenario): OceanAgent[] {
  const [, b, c, d, e] = s.boats;
  return [
    brokerAgent(b!.id, b!.name, s),
    greedyAgent(c!.id, c!.name, s),
    opportunistAgent(d!.id, d!.name, s),
    reciprocatorAgent(e!.id, e!.name, s),
  ];
}
...
for (let s = 0; s < SEEDS; s++) {
  ...
  cell.livelihood.push(rowL);
  cell.restraint.push(rowR);
  cell.cooperation.push(rowC);
}
