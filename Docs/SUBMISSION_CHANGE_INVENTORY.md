...
| `7fb651b` | 実AIの購入・Sepolia支払い・別注文返金を接続して検証 | [公開支払いEvidence](deployments/sepolia-rescue-service-demo.json)。運営管理Wallet |
| `83d3225` | 購入後の3判断、Outcome・Pool Preview、公開Replay、CRE認証Gate | [記録](deployments/rescue-submission-demo.json)。未知Final・実Pool報酬ではない |
| `5beb343` | 英語画面・動画台本、SDK Hash検証、秘密Pack・Bazanticのローカル準備 | [引継ぎ](RESCUE_HANDOFF.md)。スポンサー実接続成功ではない |
...
- Public Practiceの初回接続preflightと異常系テスト、[開発者向け手順](RESCUE_AGENT_QUICKSTART.md)。
- [情報購入の対照実験設計](RESCUE_INFORMATION_STUDY.md)と設計Hash生成。実AI比較ではない。
...
提出時は本人が確認した開始・終了Commitへ置き換え、公開Remoteに終了Commitが存在することを確認する。Pushは別指示。AI支援の記述は[英語提出草案](SUBMISSION_COPY_EN.md)を使い、実際の人間の企画・判断・実装・検証の貢献は本人の事実確認を加える。
