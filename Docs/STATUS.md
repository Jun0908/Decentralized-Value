...
| Rescue Room competition | Phase 0 returned `GO`; Phase 1 is complete; the local Controlled Practice accepts editable deterministic Doctrine Artifacts and can run a real AI Commander from a participant Playbook. Durable revision/final-entry storage, Incident Shift, committed hidden Final packs, participant uniqueness, full-field recomputation, and Sepolia Service Payment remain incomplete. Game-credit payments are simulated and explicitly not tokens. |
