...
- **Release follow-up:** [Production readiness review](PRODUCTION_READINESS_2026-09-13.md). Public pages and non-AI practice are verified; public AI availability is not. The Rescue parent route, historical payment descriptor and generated-output checks are now corrected locally (see below). Production AI configuration, Ocean execution duration/discovery and canonical list identifiers remain separate work.
...
The SDK integrity matrix now registers all 35 Episodes × 3 Doctrine presets as 105 individually named tests, retaining Vitest's default timeout and every existing hash comparison. It reads public IDs without calculating an unrelated leaderboard. `/rescue-room` redirects to `/arenas/rescue-room`; lazy Pool allocation getters are materialized into plain objects before the Server/Client boundary, with identical allocation values.

The public descriptor now reports `operator-pilot-paid` and links the recorded Sepolia evidence. Null addresses mean unconfigured Practice payment targets, not an undeployed historical contract. Practice remains `simulated` / `game-credits`; this does not enable operator-wallet access, a public payment service, or tournament rewards. SDK provenance and Starter digest were regenerated; deterministic evaluation fixtures are unchanged apart from the Starter archive digest.

README links now reflect four publicly available evidence pages (GET verified); viewing them does not run AI or make payments. Prettier/ESLint exclude CRE bundles and local recording/scratch outputs, while authored workflows and generated SDK contracts remain checked. Six existing authored files received formatting-only corrections. Other agents' commits and unrelated measurement reports are preserved.

Verification: standard `pnpm test:ts` **940 passed / 11 skipped**, without timeout or worker overrides; workspace typecheck; Web production build (41 pages); `pnpm format:check`; `pnpm exec eslint .`; SDK/CLI contract checks. `pnpm exec tsx scripts/verify-rescue-entry.ts` passed parent/three-child navigation at 1440 and 390 px with no console errors or framework error overlay. Screenshots are in ignored `.frontier/rescue-entry-qa/`. This navigation check is not a full mobile-layout audit; the existing mobile hero still has clipped text.

`pnpm lint` reaches its Foundry formatting step but cannot run `forge` in this PowerShell environment; full CI and Solidity checks are therefore not claimed. No new model call, chain transaction, credential registration, Push or deployment was performed for these corrections.
