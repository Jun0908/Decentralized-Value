...
Identity作成→Public commitment登録→実Proof受理→同じProofのHTTP409拒否という4手順の初回ミッション。各操作を導入カードから実行でき、実受理・拒否を確認したときだけ完了にする。
...
`packages/secret-gate/**`、`apps/web/src/components/secret-gate-*`、本 Plan、`Docs/product/arenas/secret-gate.md` のみ。中央 API / routes / registry / global CSS / 依存関係は変更しない。
