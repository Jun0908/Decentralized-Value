...
状態: 実装中。契約生成・配布検証とCI、Rescue限定の単一ホストDurable Job・Loopback HTTP／SDK／CLIを実装済み。2026-09-13には公開VercelでRescueの非課金API→SDKの6実HTTP、Starter・Hash・反復一致を確認した。Web公開と既存Practice接続は確認済みだが、API契約拡張全体、Ocean対応、本番Job基盤、npm公開、未知Finalは未完了。現在の境界は§10以降と[公開確認記録](../../../evidence/verification/PUBLIC_VERIFICATION_2026-09-13.md)を参照。
...
接続先のPlan9 Pilotでは、実AI Commander→Pulse Monitorの別モデル呼び出し→5 rUSD-DEMO支払い、別注文5 rUSD-DEMO返金を確認した。[公開Evidence](../../../evidence/deployments/sepolia-rescue-service-demo.json)とローカル`/rescue-room/operations`で記録を確認できる。ただし実Serviceは公開観測のinterpretation sidecarで、ゲームOutcome・診断Correctnessは新たに検証していない。Walletは運営管理。
...
2026-09-12。提出までに公開Job基盤を広げず、既存非課金Practice契約の再現手順を優先した。[API利用手順](../../../hackathon/sponsors/agent-api.md)を参照。
...
2026-09-12。計画後の実行指示により着手。横断優先順は[NEXT_UNATTENDED_BATCH.md](NEXT_UNATTENDED_BATCH.md)、実行結果は[RESCUE_HANDOFF.md](../../../hackathon/submission/RESCUE_HANDOFF.md)。
...
2026-09-12。[RESCUE_AGENT_QUICKSTART.md](../../guides/RESCUE_AGENT_QUICKSTART.md)へ起動・サンプル・各軸・異常時の対応を整理。
...
- [x] 公開後の同じコマンドによる実HTTP確認（2026-09-13）。6要求すべて200、Starter・Hash・反復一致。実参加者・Linux・npm公開・Final APIの完成とは区別する。
