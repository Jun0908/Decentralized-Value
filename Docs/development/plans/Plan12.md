...
This edition consolidates the current requirements and execution record. The [original plan](../history/plan-history-2026-09-13/Plan12.md) preserves the detailed original inventory and dated checkpoints. The current monorepo is the source of truth.
...
The public preflight on 2026-09-13 verified **non-AI doctrine practice**, not completed public inference. The table above is an implementation checkpoint, not a separate live availability matrix. Use [STATUS](../../STATUS.md) for later public observations and locally verified Ocean discovery/OpenAPI corrections, and the dated [production review](../../evidence/verification/PRODUCTION_READINESS_2026-09-13.md) for original findings. Do not reopen a corrected item based only on this older checklist.
...
The recorded 41 store/HTTP/SDK/CLI tests cover restart, ownership, duplicate requests, stale workers, ambiguous failures, and redaction. See [Plan 9 payment evidence](../../evidence/deployments/sepolia-rescue-service-demo.json). Public jobs, production DB/multi-host execution, dedicated events/cancel APIs, and external scopes remain follow-up work.
...
These are documented checks, not all rerun by this translation. Linux, real participant authentication, full public AI execution, and npm publication need separate evidence. See the [developer quickstart](../guides/RESCUE_AGENT_QUICKSTART.md) and [SDK README](../../../packages/sdk/README.md).
