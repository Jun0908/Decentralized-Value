...
5. **1:10–1:30 — Public API.** Submit the prepared interesting artifact through `/v1/evaluations`. Show the explicit `simulated` state and explain that the demo does not fabricate a hardware signature or transaction.
6. **1:30–1:45 — Optional settlement.** If configured, show the Sepolia attestation transaction and frontier update; otherwise keep the boundary visibly optional.
