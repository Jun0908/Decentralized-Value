...
状態: 実装中。契約生成・配布検証とCI、Rescue限定の単一ホストDurable Job・Loopback HTTP／SDK／CLIを実装済み。2026-09-13には公開VercelでRescueの非課金API→SDKの6実HTTP、Starter・Hash・反復一致を確認した。Web公開と既存Practice接続は確認済みだが、API契約拡張全体、Ocean対応、本番Job基盤、npm公開、未知Finalは未完了。現在の境界は§10以降と[公開確認記録](PUBLIC_VERIFICATION_2026-09-13.md)を参照。
