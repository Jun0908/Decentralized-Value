...
6. `/arenas/rescue-room` shows an ambiguous incident alert, curated Service market, an editable deterministic Doctrine or AI Playbook, replayable reason-coded Action/Payment/Evidence, three outcomes, and four Practice Value Pools. Rules and AI share the same Service, protocol-action, single-price, and total-investigation-budget gate; only the deterministic rules interpreter claims identical fresh-run behavior.
...
- Rescue Room Reference Mode is deterministic. AI Playbook Mode uses the server-side OpenAI Agents SDK with fixed model/settings and no browser-visible credential. The LLM sees only public state; model calls are non-deterministic observations, while recorded Action replay and outcome hashes are deterministic. Service Orders bind the Episode, Commander Action, Service Manifest, Deliverable, Receipt, and Acceptance with deterministic hashes. Rescue Credits remain simulated. The operator-only Pilot has deployed `RescueUSDDemo` and `RescueServiceEscrow` to Sepolia and verified a real Service payment and separate refund with operator-managed wallets; this is not a public delegated-wallet market, hidden Final, or Rescue tournament reward settlement.
- `/rescue-room/submission` continues a previously paid analysis for at most three real Commander decisions. Its Context commits the purchased-analysis snapshot, Practice/Commander constraints, metrics and fixed comparison policies. It replays recorded actions through the simulator and computes independent preview pools; no inference or payment occurs when viewing the page. A Git-tracked public fixture can reproduce the complete simulator outcome and pool allocations without private jobs, but cannot prove model provenance or payment receipts. Full runtime records remain private.
- The CRE workflow has typed regular/confidential handlers around a public Rescue fixture. Official CLI simulation currently stops at authentication. Local bundle agreement is not evidence of CRE execution or confidential computation; ENS authorization and Bazantic integration remain unverified.
